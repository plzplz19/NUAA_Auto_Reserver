...
import datetime
from encryptUserInfo import readPrikey,myDecrypt
import base64
...
def getUserInfo(key_path, data_path):
    global user_info
    pri_key = readPrikey(path=key_path)

    with open(data_path,'rb') as f:
        temp = f.readlines()
        to_be_decrypt = [base64.b64decode(i) for i in temp]
    user_info_str = myDecrypt(to_be_decrypt,pri_key).decode().replace("'",'"')
...
def reserve(hall,playTime,fieldNum):
    global session

    # 开始从 json 中取值，最后提交预约请求时候需要
    resource_id_json_expr = parse("$..list[?(@.name=='{}')].id".format(hall))
    resource_id = resource_id_json_expr.find(raw_json)[0].value
    #print(resource_id)

    data_json_expr = parse("$..list[?(@.name=='{}')]..table.*[?(@.abscissa=='{}' & @.yaxis=='{}')]".format(hall, fieldNum, playTime))
...
if __name__ == '__main__':
    # 解密客户端发来的用户信息
    getUserInfo(key_path='/Users/jinmingzhang/Programs/pyWebCrawler/n南航统一门户/GUI界面/server/private.pem',data_path='/Users/jinmingzhang/Programs/pyWebCrawler/n南航统一门户/GUI界面/server/userInfo.txt')

    # 登录(后面需要定时执行)
    login(user_info['xuehao'], user_info['password'])

    # 预约(后面需要定时执行)
    reserve(user_info['hall'], user_info['playTime'], user_info['fieldNum'])
    print(1)

#
# TODO: 
#   1.使用邮件通知用户预约结果（失败/成功）
#   2.根据用户信息定时执行任务
#
